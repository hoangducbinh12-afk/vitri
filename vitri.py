...
# --- XỬ LÝ CHÍNH ---
if uploaded_file and st.sidebar.button("Phân tích & Cập nhật điểm"):
    # Đọc ảnh
    ...
    if len(st.session_state.db["bang_a"]) != len(raw):
...
if raw:
    tab1, tab2, tab3 = st.tabs(["Bảng A (Vị trí)", "Bảng B (Thuộc tính)", "Bảng C (100 số)"])

    with tab1:
        st.subheader("Điểm tích lũy tại từng vị trí")
        df_a = pd.DataFrame([p["points"] for p in st.session_state.db["bang_a"]])
        df_a.insert(0, "Giá trị số", raw)
        st.dataframe(df_a, use_container_width=True)

    with tab2:
        st.subheader("Tổng hợp điểm theo thuộc tính (0-9)")
        ...
        df_b = pd.DataFrame(bang_b).T
        st.table(df_b)

    with tab3:
        st.subheader("Bảng điểm 100 số (Gan nhất)")
        bang_c = []
        for i in range(100):
            t = analyze_number(i)
            x, y = t["dau"], t["duoi"]
            ...
            bang_c.append({"Số": f"{i:02d}", "Tổng điểm": score})
        
        df_c = pd.DataFrame(bang_c).sort_values(by="Tổng điểm", ascending=False)
        st.dataframe(df_c, height=400, use_container_width=True)

# --- XUẤT FILE ---
st.divider()
json_string = json.dumps(st.session_state.db)
st.download_button(label="📥 Tải dữ liệu về máy (JSON)", data=json_string, file_name="loto_data.json", mime="application/json")